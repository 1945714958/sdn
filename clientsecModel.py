...
def setServiceIp(ip:str):
    print(ip)
    global server_ip
    server_ip = ip


def setLocalIp(ip:str):
    print(ip)
    global local_ip
    local_ip = ip
                ...
                if is_msg_authenticated(rcv_message, controller_shared_key):
                    ...
                    if unpacked_auth_reply['code'] == "202 Authentication Accepted":
                        ...
                        if max_nb_attempts < 16:
                            print('\tUpdating counter to {} and increasing max_nb_attempts value'.format(
                                unpacked_auth_reply['expected_ctr']))
                            ctr = unpacked_auth_reply['expected_ctr']
