...
var shared_esm_bundler_exports2 = {};
__export(shared_esm_bundler_exports2, {
  assign: () => assign,
...
var inBrowser, mark, measure, RE_ARGS, hasSymbol, makeSymbol, generateFormatCacheKey, friendlyJSONstringify, isNumber, isDate, isRegExp, isEmptyObject, assign, _globalThis, getGlobalThis, hasOwnProperty, isArray2, isFunction, isString2, isBoolean, isSymbol2, isObject2, isPromise, objectToString, toTypeString, isPlainObject, toDisplayString, RANGE;
...
function handleFlatJson(obj) {
  ...
  for (const key in obj) {
    ...
    if (!key.includes(".")) {
      ...
      delete obj[key];
      if (isObject3(currentObj[subKeys[lastIndex]])) {
...
var hasOwnProperty2, isObject3, pathStateMachine, literalValueRE, cache;
...
function createMessageContext(options = {}) {
  ...
  const pluralRule = isObject2(options.pluralRules) && isString2(locale) && isFunction(options.pluralRules[locale]) ? options.pluralRules[locale] : pluralDefault;
  const orgPluralRule = isObject2(options.pluralRules) && isString2(locale) && isFunction(options.pluralRules[locale]) ? pluralDefault : void 0;
  ...
  function message(key) {
    const msg = isFunction(options.messages) ? options.messages(key) : isObject2(options.messages) ? options.messages[key] : false;
...
function createCoreContext(options = {}) {
  ...
  const fallbackLocale = isArray2(options.fallbackLocale) || isPlainObject(options.fallbackLocale) || isString2(options.fallbackLocale) || options.fallbackLocale === false ? options.fallbackLocale : locale;
...
function getLocaleChain(ctx, fallback, start) {
  ...
  if (!chain) {
    ...
    const defaults = isArray2(fallback) ? fallback : isPlainObject(fallback) ? fallback["default"] ? fallback["default"] : null : fallback;
...
function createCompilerError(code, loc, messages, additionalMessage) {
  const msg = true ? (messages || errorMessages3)[code] + (additionalMessage || ``) : code;
  const error = new SyntaxError(String(msg));
  error.code = code;
  error.loc = loc;
  return error;
...
function createTransformContext(root, { filename = "", prefixIdentifiers = false, hoistStatic: hoistStatic2 = false, cacheHandlers = false, nodeTransforms = [], directiveTransforms = {}, transformHoist = null, isBuiltInComponent = NOOP, isCustomElement = NOOP, expressionPlugins = [], scopeId = null, slotted = true, ssr = false, inSSR = false, ssrCssVars = ``, bindingMetadata = EMPTY_OBJ, inline = false, isTS = false, onError = defaultOnError2, onWarn = defaultOnWarn, compatConfig }) {
...
function compile(template, options = {}) {
  return baseCompile2(template, extend({}, parserOptions, options, {
    nodeTransforms: [
...
var require_vue_i18n_cjs = __commonJS({
  "node_modules/vue-i18n/dist/vue-i18n.cjs.js"(exports) {
    Object.defineProperty(exports, "__esModule", { value: true });
    var shared = (init_shared_esm_bundler2(), __toCommonJS(shared_esm_bundler_exports2));
    ...
    function createI18nError(code, ...args) {
      return coreBase.createCompileError(code, null, { messages: errorMessages4, args });
    }
    var errorMessages4 = {
