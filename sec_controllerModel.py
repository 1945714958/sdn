...
class ZodiacSwitch(app_manager.RyuApp):
    ...
    def add_flow(
        self, datapath, priority, match, actions, idle_timeout=0, buffer_id=None
    ):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def make_auth_reply(self, code, expected_ctr, shared_key):
        # json request
        reply_data_dict = {"code": code, "expected_ctr": expected_ctr, "hmac": 0}
    ...
    def _packet_in_handler(self, ev):
        ...
        self.net.add_edges_from(links)
        links_ = [
            (link.dst.dpid, link.src.dpid, link.dst.port_no) for link in links_list
        ]
        for l in links_:
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            if arp_packet.opcode == 1:
                ...
                if arp_dst_ip == self.controller_ip:
                    ...
                    self.send_arp(
                        datapath, opcode, srcMac, srcIp, dstMac, dstIp, outPort
                    )
                ...
                self.send_arp(
                    datapath_dst, opcode, srcMac, srcIp, dstMac, dstIp, outPort
                )
        ...
        if ip4_pkt:
            ...
            if src_ip not in self.servers_ip and proto == "17":
                ...
                if correct:
                    authenticated = self.is_msg_authenticated(
                        unpacked_auth_request, client_shared_key
                    )
                    if authenticated:
                        ...
                        if rcv_ctr in range(
                            self.expected_ctr, self.expected_ctr + self.ctr_margin
                        ):
                ...
                # ----- Send the auth reply -----
                # Generate auth reply
                auth_reply = self.make_auth_reply(
                    code, self.expected_ctr, controller_shared_key
                )

                # Create the packet ready to be sent
                reply_pkt = (
                    Ether(src=dst, dst=src)
                    / IP(src=dst_ip, dst=src_ip)
                    / UDP(sport=int(dport), dport=int(sport))
                    / Raw(load=auth_reply)
                )
                reply_pkt.build()
            ...
            if authorised:
                ...
                try:
                    if src_ip not in self.servers_ip:
                        datapath_dst = get_datapath(
                            self,
                            self.mac_to_dpid[
                                self.ip_to_mac[unpacked_auth_request["server"]]
                            ],
                        )
                    ...
                    self.logger.info(
                        " --- No informations on how to reach %s",
                        unpacked_auth_request["server"],
                    )
                    return
                ...
                if len(path) == 1:
                    ...
                    if src_ip not in self.servers_ip:
                        if unpacked_auth_request["tcp_udp"] == "tcp":
                            match_1 = parser.OFPMatch(
                                ...
                                ipv4_dst=dst_ip,
                                ip_proto=6,
                                tcp_dst=unpacked_auth_request["dport"],
                            )
                            match_2 = parser.OFPMatch(
                                ...
                                ipv4_dst=src_ip,
                                ip_proto=6,
                                tcp_src=unpacked_auth_request["dport"],
                            )
                        else:
                            match_1 = parser.OFPMatch(
                                in_port=In_Port,
                                ...
                                ipv4_dst=dst_ip,
                                ip_proto=17,
                                udp_dst=unpacked_auth_request["dport"],
                            )
                            match_2 = parser.OFPMatch(
                                in_port=Out_Port,
                                eth_dst=src,
                                ...
                                ipv4_dst=src_ip,
                                ip_proto=17,
                                udp_src=unpacked_auth_request["dport"],
                            )
                    else:
                    ...
                    self.add_flow(
                        ...
                        match_1,
                        actions_1,
                        idle_timeout=self.flows_expire_in,
                    )
                    self.add_flow(
                        datapath,
                        1,
                    ...
                    Out_Port_dst = self.net[path[len(path) - 1]][path[len(path) - 2]][
                        "port"
                    ]
                    ...
                    if src_ip not in self.servers_ip:
                        if unpacked_auth_request["tcp_udp"] == "tcp":
                            match_2_src = parser.OFPMatch(
                                in_port=Out_Port_src,
                                eth_type=0x0800,
                                ipv4_src=dst_ip,
                                ipv4_dst=src_ip,
                                ip_proto=6,
                    ...
                    match_2_dst = parser.OFPMatch(
                        in_port=Out_Port_dst,
                        eth_type=0x0800,
                        ipv4_src=src_ip,
                        ipv4_dst=dst_ip,
                    )
                    ...
                    self.logger.info(
                        "Install the flow on switch %s", path[len(path) - 1]
                    )
                ...
                if src_ip in self.servers_ip:
                    ...
                    if not authorised:
                        self.logger.info(
                            "Packet not authorised from %s to %s", src_ip, dst_ip
                        )
...
def run():
    ...
    print(
        "Obtained the following shared keys:\n\tclient {}\n\tcontroller {}".format(
            client_shared_key.hexdigest(), controller_shared_key.hexdigest()
        )
    )
